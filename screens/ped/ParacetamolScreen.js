 ...
 * não vimos o alvo real de mg/kg usado no seu app. Usei 12 mg/kg/dose
 * (dentro da faixa clássica de 10–15 mg/kg/dose) como suposição — ajuste
 ...
 * gotejador padrão de 20 gotas/mL) — o print não informa isso explicitamente
 * como fez com Dipirona e Tramadol.
 */

const TARGET_MG_PER_KG = 12;
const RANGE_TEXT = '10-15 mg/kg/dose';

const DROPS_PER_ML = 20; // suposição — não informado no print
const GOTAS_MG_ML = 200;
const MG_PER_DROP = GOTAS_MG_ML / DROPS_PER_ML;
const MAX_DROPS = 35;

const SUSP32_MG_ML = 32;
const MAX_ML_SUSP32 = 20;

const SUSP100_MG_ML = 100;
const MAX_ML_SUSP100 = 6.4;

function roundToStep(value, step) {
  return Math.round(value / step) * step;
}

export default function ParacetamolScreen() {
  ...
  const doses = useMemo(() => {
    ...
    const susp32Ml = Math.min(roundToStep(targetMg / SUSP32_MG_ML, 0.5), MAX_ML_SUSP32);
    const susp100Ml = Math.min(roundToStep(targetMg / SUSP100_MG_ML, 0.1), MAX_ML_SUSP100);

    return {
      drops,
      susp32Ml: susp32Ml.toFixed(1).replace(/\.0$/, ''),
      susp100Ml: susp100Ml.toFixed(1),
    };
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.formulationsBlock}>
          <FormulationSection
            heading={`Solução oral (gotas) ${GOTAS_MG_ML} mg/mL:`}
          ...
          <FormulationSection
            heading={`Suspensão oral ${SUSP32_MG_ML} mg/mL:`}
            highlighted={doses ? `${doses.susp32Ml} mL` : '00 mL'}
            instruction="via oral a cada 6 horas se dor ou febre"
            subItems={[
              `dose máxima de ${MAX_ML_SUSP32} mL por dose`,
              'apresentação em frasco com 60 mL',
            ]}
          />

          <FormulationSection
            heading={`Suspensão oral ${SUSP100_MG_ML} mg/mL:`}
            highlighted={doses ? `${doses.susp100Ml} mL` : '00 mL'}
            instruction="via oral a cada 6 horas se dor ou febre"
            subItems={[
              `dose máxima de ${MAX_ML_SUSP100} mL por dose`,
              'apresentação em frasco com 15 mL',
            ]}
          />
        </View>
        ...
        <View style={styles.observationsBlock}>
          ...
          <ObservationItem>
            Dose máxima diária de 75 mg/kg (limitado a 4 g/dia)
          </ObservationItem>

          <ObservationItem>
            Intervalo mínimo de 4-6 horas entre as doses
          </ObservationItem>
        </View>
      </ScrollView>
    </SafeAreaView>
